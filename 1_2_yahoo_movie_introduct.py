...
def movie_Introduction(id):
    ...
    with open('2_movie_about.csv', 'a', newline='', encoding='utf-8') as csvfile:
...
def main():
    data = pd.read_csv('1_movie.csv')
    id_list_1 = int(list(data['電影ID'])[-1])+1



    if not os.path.isfile('2_movie_about.csv'):
        ...
        data2=pd.read_csv('2_movie_about.csv')
    ...
    for id in range(id_list_2,id_list_1):
        try:
            ...
            print(id,'未完成')
            with open('2_movie_about_lose.csv', 'a', newline='', encoding='utf-8') as f:
                f.write(str(id)+'\n')
            continue
